...
public class Column {
	 ...
	 * @return
	 */
	public String getRemark() {  // 从_rearks获取
		...
		if(remark!=null){
			...
			if(_remarks==null){
				...
				int indexOf = _remarks.indexOf(":");
				if(indexOf==-1){
					remark=_remarks;
				}else{
					remark=_remarks.substring(0,indexOf);
				}
			}
		}
...
//			_table.getExportedKeys().addForeignKey(refTable.getSqlName(), hasMany.columnSqlName, getSqlName(), hasMany.columnSqlName.toLowerCase().hashCode());
	...
	private void initOtherProperties() {
		...
		//normalJdbcJavaType 是java.sql.数据类型 ,javaType是上面的值通过配置文件转换过来的,没写的默认和上面相同! 
		String normalJdbcJavaType = DatabaseDataTypesUtils.getPreferredJavaType(sqlType, size, decimalDigits);
		javaType = GeneratorProperties.getProperty("java_typemapping."+normalJdbcJavaType,normalJdbcJavaType).trim();  //转换属性
